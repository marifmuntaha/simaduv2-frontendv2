...
import {get as getAccounts, store as storeAccount, update as updateAccount} from "@/api/finance/account"
...
const Partial = ({modal, setModal, account, setAccount, setReloadData}) => {
    ...
    const onStore = async () => {
        ...
        const formData = {
            ...
            codeParent: account.codeParent,
            codeApp: account.codeParent + account.code,
            code: account.code,
            name: account.name,
            level: account.level,
    ...
    useEffect(() => {
        account.institutionId !== undefined && getAccounts({type: 'select', 'institutionId': account.institutionId}).then(data => {
                        ...
                        <div className="form-group col-md-6">
                            ...
                            <div className="form-control-wrap">
                                <RSelect
                                    ...
                                    value={accountOptions?.find((c) => c.value === account.id)}
                                    onChange={(e) => {
                                        setAccount({...account, codeParent: e.value});
                                    }}
